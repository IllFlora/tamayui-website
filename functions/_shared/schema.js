...
export function requireBinding(env, binding) {
  if (!env?.[binding]) {
    throw new SetupRequiredError(`Cloudflare binding ${binding} is not configured.`);
  }
  return env[binding];
}
...
export async function ensureSchema(env) {
  ...
  if (!schemaPromise) {
    schemaPromise = db.batch(SCHEMA_STATEMENTS.map((statement) => db.prepare(statement))).catch((error) => {
